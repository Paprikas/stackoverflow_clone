...
RSpec.describe CommentsController, type: :controller do
  ...
  let(:comment_answer) do
    post :create, xhr: true, params: {commentable: 'answer', commentable_id: answer, comment: attributes_for(:comment)}
  end

  let(:invalid_comment_answer) do
    post :create, xhr: true, params: {commentable: 'answer', commentable_id: answer, comment: attributes_for(:comment, :invalid)}
  end
  ...
  context 'as user' do
    ...
    describe "POST #create" do
      ...
      it "assigns comment to @comment" do
        comment_answer
        expect(assigns(:comment)).to eq answer.comments.first
      end
      ...
      it 'creates comment' do
        expect { comment_answer }.to change(answer.comments, :count).by(1)
      end

      it "doesn't creates invalid comment" do
        expect { invalid_comment_answer }.not_to change(answer.comments, :count)
      end
